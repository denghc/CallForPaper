 ...
        alert(evt.target.error.code);
    }	
	...
	function prependNews(data){
		...
		if(thumiao.PhoneSencha.array!=null && thumiao.PhoneSencha.array.length == 2){
	...
	this.oneNews = function(record){
		...
		var curUserUserimageLink = data2.userimage;
	...
	function replysAll(id,result){
		...
		o.refresh();
		//
		thumiao.PhoneSencha.curRecord.data['comments'] = result.length;
		var list = Ext.getCmp('thu_newsList');
		list.refresh();
	}
	//
	...
	this.noticeOne = function(record){
		...
		$('#thumiao_notice_content').html(data['detail']);
	}
	//------------------------------------------
	//vote functions
	//------------------------------------------
