...
ThuMiao.prototype.PhoneSenchaWS = new function(){
	...
	function initColor(color,x,y){
		...
		color = "'"+color+"'";
		var onclick="onclick=thumiao.PhoneSenchaWS.setColor("+color+") ";
	...
	function initWidth(width,x,y){
		...
		var onclick="onclick=thumiao.PhoneSenchaWS.setWidth("+width+") ";
		var temp="<div class='thu_CanvasColor' "+style+onclick;
		temp+=">"+width+"</div>";
		return temp;
	}
	...
	function getT(data){
		...
		if(thumiao.PhoneSenchaWS.isFocusTags == false){
			temp+=' onclick="thumiao.PhoneSenchaWS.setT(';
			temp+=data['id']+')"';
		}
		else{
			temp+=' onclick="thumiao.PhoneSenchaWS.setFocusT(';
			temp+=data['id']+')"';
		}
	...
	function getAskData(data){
		...
		data['rawtags'] = data['tags'];
		data['tags'] = thumiao.PhoneSenchaWS.dressTags(data['tags']);
		data['value'] = 0;
		return data;
	}
	...
	this.setAskCur = function(record){
		...
		temps = '<div class="newsitem"><table width="480" border="0">  <tr>    <td width="66" rowspan="2"><div class="item-userimage-style1" align="center"><img src="' + record.data.headurl + '" width="36" height="36" /></div></td>    <td class="topic-style1" width="318" height="34"><font face="微软雅黑"><strong>' + record.data.title + '</strong></font><font face="微软雅黑" size="2">&nbsp;&nbsp;——' + record.data.name + '</font></td>  </tr> </table></div>';
		o.setHtml(temps);
		//
